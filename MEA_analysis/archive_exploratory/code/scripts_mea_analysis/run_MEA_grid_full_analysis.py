...
核心思想：
1. single-edge 与 double-edge 来自不同 recording / spike sorting，不能进行 unit-level pairing。
...
3. 因此本脚本采用 spatially matched grid-level local population analysis：
   在同一视网膜切片的相同空间网格内，分别聚合 single 和 double 条件下的 good sorted units，
...
OUT_DIR = PROJECT_DIR / "outputs" / "MEA_analysis" / "MEA_analysis_final"
...
PAIR_CONFIG = {
    "pair_31_32": {"double": "000031", "single": "000032"},
    "pair_34_35": {"double": "000034", "single": "000035"},
    "pair_37_38": {"double": "000037", "single": "000038"},
}
...
def classify_delta(delta: pd.Series, valid: pd.Series, threshold: float) -> pd.Series:
    """按指定阈值将网格差异分为 single_higher / double_higher / similar。"""

    result = pd.Series("similar", index=delta.index, dtype=object)
    result[delta > threshold] = "single_higher"
    result[delta < -threshold] = "double_higher"
...
def load_unit_positions(exp_id: str, stim_class: str) -> pd.DataFrame:
    """读取一个实验的 good unit 空间坐标。

    unit_distance_long_table.csv 中同一个 unit 会在多个方向重复出现，因此需要 drop_duplicates。
    unit_row_idx 是 good_on/output_fre.npy 的 unit 轴索引。
    """

    path = GEOMETRY_DIR / stim_class / exp_id / "good" / "data" / f"{exp_id}_unit_distance_long_table.csv"
    if not path.exists():
        raise FileNotFoundError(f"缺少 unit 坐标文件: {path}")
    table = pd.read_csv(path)
    required = {"unit_row_idx", "cluster_id", "x", "y"}
    missing = required - set(table.columns)
    if missing:
        raise ValueError(f"{path} 缺少字段: {missing}")
    units = (
        table[["unit_row_idx", "cluster_id", "x", "y"]]
        .drop_duplicates("unit_row_idx")
        .sort_values("unit_row_idx")
        .reset_index(drop=True)
    )
    if units.empty:
        raise ValueError(f"{exp_id} 没有可用 good unit 坐标")
    return units
...
def assign_grid(units: pd.DataFrame, x_edges: np.ndarray, y_edges: np.ndarray, grid_n: int) -> pd.DataFrame:
    ...
    同一 pair、同一 grid_scale 下 single 和 double 使用相同 x_edges/y_edges。
    """
...
def compute_one_grid_direction(
    ...
    double_exp = pair_cfg["double"]
    single_exp = pair_cfg["single"]
    direction_zero_based = direction["id"] - 1

    single_units = load_unit_positions(single_exp, "single_edge")
    double_units = load_unit_positions(double_exp, "double_edge")

    # 使用 single + double 的共同坐标范围确定网格边界，避免两组各自坐标范围不同导致不可比。
    all_x = pd.concat([single_units["x"], double_units["x"]], ignore_index=True)
    all_y = pd.concat([single_units["y"], double_units["y"]], ignore_index=True)
    x_edges = np.linspace(all_x.min(), all_x.max(), grid_n + 1)
    y_edges = np.linspace(all_y.min(), all_y.max(), grid_n + 1)
    single_units = assign_grid(single_units, x_edges, y_edges, grid_n)
    double_units = assign_grid(double_units, x_edges, y_edges, grid_n)

    single_fr = load_middle_step_fr(single_exp, "single", direction_zero_based)
    double_fr = load_middle_step_fr(double_exp, "double", direction_zero_based)
    single_unit_table = build_unit_table(single_units, single_fr, "single")
    double_unit_table = build_unit_table(double_units, double_fr, "double")
    unit_df = pd.concat([single_unit_table, double_unit_table], ignore_index=True)

    single_grid = aggregate_grid(single_unit_table, "single")
    double_grid = aggregate_grid(double_unit_table, "double")
    ...
    grid = full_grid.merge(single_grid, on=["grid_id", "grid_x", "grid_y"], how="left")
    grid = grid.merge(double_grid, on=["grid_id", "grid_x", "grid_y"], how="left")
    ...
    for col in [
        ...
        "single_zero_count",
        "double_zero_count",
        "single_nonzero_count",
        "double_nonzero_count",
    ]:
    ...
    grid["single_exp"] = single_exp
    grid["double_exp"] = double_exp
    ...
    grid["valid_grid"] = (
        (grid["single_unit_count"] >= MIN_UNITS_PER_GRID_PER_STIM)
        & (grid["double_unit_count"] >= MIN_UNITS_PER_GRID_PER_STIM)
    )

    # 均值、median、nonzero FR、response fraction 差异。
    grid["delta_mean_fr_hz"] = grid["single_mean_fr_hz"] - grid["double_mean_fr_hz"]
    grid["abs_delta_mean_fr_hz"] = grid["delta_mean_fr_hz"].abs()
    grid["delta_median_fr_hz"] = grid["single_median_fr_hz"] - grid["double_median_fr_hz"]
    grid["abs_delta_median_fr_hz"] = grid["delta_median_fr_hz"].abs()
    grid["delta_nonzero_fraction"] = grid["single_nonzero_fraction"] - grid["double_nonzero_fraction"]
    grid["abs_delta_nonzero_fraction"] = grid["delta_nonzero_fraction"].abs()
    grid["delta_zero_fraction"] = grid["single_zero_fraction"] - grid["double_zero_fraction"]
    grid["abs_delta_zero_fraction"] = grid["delta_zero_fraction"].abs()
    grid["delta_nonzero_mean_fr_hz"] = grid["single_nonzero_mean_fr_hz"] - grid["double_nonzero_mean_fr_hz"]
    grid["abs_delta_nonzero_mean_fr_hz"] = grid["delta_nonzero_mean_fr_hz"].abs()
    grid["delta_nonzero_median_fr_hz"] = grid["single_nonzero_median_fr_hz"] - grid["double_nonzero_median_fr_hz"]
    grid["abs_delta_nonzero_median_fr_hz"] = grid["delta_nonzero_median_fr_hz"].abs()

    # NDI 反映相对 single/double 偏好，范围理论上接近 [-1, 1]。
    grid["NDI_mean"] = ndi(grid["single_mean_fr_hz"], grid["double_mean_fr_hz"])
    grid["NDI_median"] = ndi(grid["single_median_fr_hz"], grid["double_median_fr_hz"])
    grid["NDI_nonzero_mean"] = ndi(grid["single_nonzero_mean_fr_hz"], grid["double_nonzero_mean_fr_hz"])
    grid["NDI_nonzero_fraction"] = ndi(grid["single_nonzero_fraction"], grid["double_nonzero_fraction"])
    ...
    unit_df["single_exp"] = single_exp
    unit_df["double_exp"] = double_exp
...
def summarize_threshold(grid_all: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    for threshold in THRESHOLDS_HZ:
        ...
        for keys, sub in grid_all.groupby(["grid_scale", "grid_n", "pair_id", "direction_id", "direction_code", "direction_name"]):
            ...
            single_n = int(counts.get("single_higher", 0))
            double_n = int(counts.get("double_higher", 0))
            similar_n = int(counts.get("similar", 0))
            diff_n = single_n + double_n
            rows.append(
                {
                    "grid_scale": keys[0],
                    "grid_n": keys[1],
                    ...
                    "valid_grid_count": n,
                    "single_higher_count": single_n,
                    "double_higher_count": double_n,
                    "similar_count": similar_n,
                    "different_count": diff_n,
                    "single_higher_fraction": single_n / n if n else np.nan,
                    "double_higher_fraction": double_n / n if n else np.nan,
    ...
    pair_overall = (
        ...
        .agg(
            ...
            mean_single_higher_fraction=("single_higher_fraction", "mean"),
            mean_double_higher_fraction=("double_higher_fraction", "mean"),
            mean_similar_fraction=("similar_fraction", "mean"),
            total_valid_grids=("valid_grid_count", "sum"),
            total_single_higher=("single_higher_count", "sum"),
            total_double_higher=("double_higher_count", "sum"),
...
def summarize_map_similarity(grid_all: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame]:
    """计算 single_map 与 double_map 的空间相似性。"""

    rows = []
    for keys, sub in grid_all[grid_all["valid_grid"]].groupby(
        ["grid_scale", "grid_n", "pair_id", "direction_id", "direction_code", "direction_name"]
    ):
        single = sub["single_mean_fr_hz"].to_numpy(dtype=float)
        double = sub["double_mean_fr_hz"].to_numpy(dtype=float)
        delta = single - double
        pearson_r, pearson_p = pearson_with_approx_p(single, double)
        spearman_r, spearman_p = spearman_with_approx_p(single, double)
        cos = cosine_similarity(single, double)
        mae = float(np.mean(np.abs(delta))) if delta.size else np.nan
        rmse = float(np.sqrt(np.mean(delta**2))) if delta.size else np.nan
        mean_level = float(np.mean((single + double) / 2)) if delta.size else np.nan
        ...
        rows.append(
            {
                ...
                "mean_single_unit_count_per_valid_grid": safe_mean(valid["single_unit_count"]),
                "mean_double_unit_count_per_valid_grid": safe_mean(valid["double_unit_count"]),
                "median_single_unit_count_per_valid_grid": safe_median(valid["single_unit_count"]),
                "median_double_unit_count_per_valid_grid": safe_median(valid["double_unit_count"]),
            }
        )
...
def permutation_null_for_combo(
    ...
    """在每个有效网格内随机打乱 single/double 标签，构建 null distribution。
    ...
    for grid_id in grid_valid["grid_id"]:
        ...
        single_n = int((values["stimulus"] == "single").sum())
        arr = values["firing_rate_hz"].to_numpy(dtype=float)
        if single_n >= MIN_UNITS_PER_GRID_PER_STIM and (arr.size - single_n) >= MIN_UNITS_PER_GRID_PER_STIM:
            grid_arrays.append((arr, single_n))
    ...
    for i in range(N_PERM):
        deltas = []
        ndis = []
        for arr, single_n in grid_arrays:
...
def make_delta_and_classification_map(grid: pd.DataFrame) -> None:
    ...
    save_figure(
        ...
        FigureRecord(
            ...
            file_png="",
            file_pdf="",
            description="Grid-level single_mean_fr_hz - double_mean_fr_hz heatmap; S/D/= are threshold-based descriptive classes.",
        ),
    )
...
def make_response_fraction_map(grid: pd.DataFrame) -> None:
    ...
    ax.set_title(f"{grid_scale} {pair_id} dir-{direction_id} {direction_code}\nDelta responsive fraction: single - double")
    ax.set_xlabel("grid x")
    ax.set_ylabel("grid y")
    ...
    save_figure(
        ...
        FigureRecord(
            ...
            file_png="",
            file_pdf="",
            description="Responsive-unit fraction difference map; positive means single has larger responsive fraction.",
        ),
    )
...
def make_map_similarity_scatter(grid: pd.DataFrame) -> None:
    """single_map vs double_map scatter plot。"""
    ...
    direction_id = grid["direction_id"].iloc[0]
    direction_code = grid["direction_code"].iloc[0]
    x = valid["single_mean_fr_hz"].to_numpy(dtype=float)
    y = valid["double_mean_fr_hz"].to_numpy(dtype=float)
    ...
    ax.set_xlim(0, lim)
    ax.set_ylim(0, lim)
    ax.set_xlabel("single mean FR (Hz)")
    ax.set_ylabel("double mean FR (Hz)")
    ax.set_title(f"{grid_scale} {pair_id} dir-{direction_id} {direction_code}\nr={r:.2f}, n={len(valid)}")
    save_figure(
        fig,
        "map_similarity",
        f"{grid_scale}_{pair_id}_dir-{direction_id}_{direction_code}_single_vs_double_scatter",
        FigureRecord(
            figure_id=f"{grid_scale}_{pair_id}_{direction_id}_scatter",
            figure_type="single_double_map_scatter",
            grid_scale=grid_scale,
            pair_id=pair_id,
            direction_id=direction_id,
            file_png="",
    ...
    module_rows.append(
        {
            ...
            "paper_value": "有利",
            "reason": "若 NDI 也不低，说明差异不只是绝对 firing-rate 高低造成，也包含相对 single/double 偏好变化。",
        }
    )
    ...
    module_rows.append(
        {
            ...
            "paper_value": "有利",
            "reason": "直接量化 single_map 与 double_map 的空间响应图差异，非常适合连接 RBCM 空间上下文调制动机。",
        }
    )
...
本分析比较的是 single-edge 和 double-edge 在同一视网膜切片中空间匹配局部区域内诱发的 RGC sorted-unit 群体响应模式，而不是同一 RGC 的 paired response。由于 single-edge 和 double-edge 来自不同 recording / spike sorting 结果，sorted units 无法可靠一一对应，因此本分析采用 spatially matched grid-level local population analysis。

## 2. 数据与实验配对

| pair_id | double-edge recording | single-edge recording |
|---|---|---|
...
- 使用 good units。
- 只分析 ON phase。
- 只取边缘移动到图像中间的步骤：single-edge 第 7/13 步，double-edge 第 6/11 步。
- 每个 unit 的 firing rate 为 3 次 repeat 的平均值。
- 分析 8 个方向。
- 在同一 paired retina 内划分空间网格，对同一空间网格内的 single 和 double units 分别聚合。
...
```text
single_unit_count >= {MIN_UNITS_PER_GRID_PER_STIM} 且 double_unit_count >= {MIN_UNITS_PER_GRID_PER_STIM}
```

主差异值：

```text
delta_mean_fr_hz = single_mean_fr_hz - double_mean_fr_hz
```
...
{markdown_table(main_pair[["grid_scale", "pair_id", "mean_different_fraction", "min_different_fraction", "max_different_fraction", "mean_abs_delta_mean_fr_hz", "mean_abs_NDI_mean", "mean_normalized_MAE", "total_valid_grids", "total_single_higher", "total_double_higher", "total_similar"]])}
...
- pair_37_38 中等且较稳定。
- single_higher 和 double_higher 均存在，说明不是简单全局 gain change，而是 spatially heterogeneous modulation。
...
{markdown_table(threshold_main[["grid_scale", "pair_id", "threshold_hz", "mean_different_fraction", "mean_single_higher_fraction", "mean_double_higher_fraction", "mean_similar_fraction", "total_valid_grids"]])}
...
response fraction 通过 `delta_nonzero_fraction = single_nonzero_fraction - double_nonzero_fraction` 衡量。完整结果见 `robust_metric_grid_summary.csv` 和 grid-cell 明细表。
...
NDI 用于衡量相对 single/double 偏好，避免只依赖绝对 firing-rate 差值。完整结果见 `NDI_summary.csv`。
...
spatial response map similarity 直接比较 single_map 与 double_map 是否相似。完整结果见 `spatial_map_similarity.csv` 和 `spatial_map_similarity_pair_overall.csv`。
...
Permutation null 在每个有效网格内随机打乱 single/double 标签，保持原始 sample size 不变，检验真实标签下的 mean_abs_delta / different_fraction 是否大于随机标签预期。完整结果见 `permutation_null_summary.csv`。
...
{markdown_table(valid_counts[valid_counts["grid_n"].isin(MAIN_GRID_SCALES_FOR_FIGURES)][["grid_scale", "pair_id", "direction_id", "direction_code", "total_grid_count", "valid_grid_count", "valid_grid_fraction", "mean_single_unit_count_per_valid_grid", "mean_double_unit_count_per_valid_grid"]], max_rows=30)}
...
> MEA 记录结果显示，single-edge 与 double-edge 刺激在同一视网膜切片的空间匹配网格中诱发了不同的局部 RGC 群体响应模式。在 3 组 paired retina 和多种网格尺度下，均有相当比例的有效网格表现出超过预设阈值的 firing-rate 差异。同时，single 优势区域和 double 优势区域均存在，说明这种差异并不是简单的全局增益变化，而是具有空间异质性的 RGC 群体响应调制。该结果提示边缘上下文能够改变视网膜空间群体响应模式，并为后续构建视网膜启发的边缘上下文调制模块提供了生物学动机。

English:

> MEA recordings showed that single-edge and double-edge stimulation evoked distinct local RGC population response patterns in spatially matched retinal grids. Across three paired retinal preparations and multiple grid resolutions, a substantial fraction of valid grids showed firing-rate differences exceeding predefined thresholds. Both single-dominant and double-dominant regions were observed, indicating that the effect was not a simple global gain change but a spatially heterogeneous modulation of RGC population activity. These observations suggest that edge context can reshape retinal population responses and motivated the design of a retinal-inspired boundary context modulation module for robust edge detection.
